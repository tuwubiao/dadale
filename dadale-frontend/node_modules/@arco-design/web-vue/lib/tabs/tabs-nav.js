  ...
  setup(props, {
    ...
    const setActiveTabOffset = () => {
      if (!activeTabRef.value || !wrapperRef.value || !isScroll.value)
        return;
      utils.updateScrollOffset(wrapperRef.value, direction.value);
      ...
      if (scrollPosition.value === "auto") {
        ...
        setOffset(tabOffset + tabSize - wrapperSize + tabMargin);
      } else if (is.isNumber(scrollPosition.value)) {
    ...
    vue.watch([activeIndex, scrollPosition], () => {
      setTimeout(() => {
        setActiveTabOffset();
      }, 0);
    });
    ...
    const listStyle = vue.computed(() => utils.getTabListStyle({
